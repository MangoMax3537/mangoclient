'use strict';

/**
 * Mojang's official classic Steve skin template, embedded as a data URL so
 * account-less and offline launcher sessions never depend on the network.
 * Source: https://assets.mojang.com/SkinTemplates/steve.png
 */
const STEVE_PNG_BASE64 = '';
const STEVE_DATA_URL = `data:image/png;base64,${STEVE_PNG_BASE64}`;

function defaultSkinDataUrl() {
  return STEVE_DATA_URL;
}

module.exports = { defaultSkinDataUrl };
